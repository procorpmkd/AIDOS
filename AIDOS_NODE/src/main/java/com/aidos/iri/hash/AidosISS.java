 ...
 */
public class AidosISS {

    public static final int NUMBER_OF_FRAGMENT_CHUNKS = 27;
    private static final int FRAGMENT_LENGTH = AidosCurl.HASH_LENGTH * NUMBER_OF_FRAGMENT_CHUNKS;
    ...
    public static int[] subseed(final int[] seed, int index) {
        ...
        final int[] subseed = new int[AidosCurl.HASH_LENGTH];

        final AidosCurl hash = new AidosCurl();
    ...
    public static int[] key(final int[] subseed, final int numberOfFragments) {
        ...
        if (subseed.length != AidosCurl.HASH_LENGTH) {
    ...
    public static int[] digests(final int[] key) {
        ...
        final int[] digests = new int[key.length / FRAGMENT_LENGTH * AidosCurl.HASH_LENGTH];
        ...
        for (int i = 0; i < key.length / FRAGMENT_LENGTH; i++) {
            ...
            for (int j = 0; j < NUMBER_OF_FRAGMENT_CHUNKS; j++) {
                ...
                for (int k = MAX_TRYTE_VALUE - MIN_TRYTE_VALUE; k-- > 0; ) {
                    final AidosCurl hash = new AidosCurl();
                    hash.absorb(buffer, j * AidosCurl.HASH_LENGTH, AidosCurl.HASH_LENGTH);
                    hash.squeeze(buffer, j * AidosCurl.HASH_LENGTH, AidosCurl.HASH_LENGTH);
                }
            }
            final AidosCurl hash = new AidosCurl();
            hash.absorb(buffer, 0, buffer.length);
            hash.squeeze(digests, i * AidosCurl.HASH_LENGTH, AidosCurl.HASH_LENGTH);
        }
    ...
    public static int[] address(final int[] digests) {
        ...
        if (digests.length == 0 || digests.length % AidosCurl.HASH_LENGTH != 0) {
            throw new RuntimeException("Invalid digests length: " + digests.length);
        }

        final int[] address = new int[AidosCurl.HASH_LENGTH];
    ...
    public static int[] normalizedBundle(final int[] bundle) {
        ...
        if (bundle.length != AidosCurl.HASH_LENGTH) {
            throw new RuntimeException("Invalid bundle length: " + bundle.length);
        }

        final int[] normalizedBundle = new int[AidosCurl.HASH_LENGTH / TRYTE_WIDTH];

        for (int i = 0; i < NUMBER_OF_SECURITY_LEVELS; i++) {

            int sum = 0;
            for (int j = i * (AidosCurl.HASH_LENGTH / TRYTE_WIDTH / NUMBER_OF_SECURITY_LEVELS); j < (i + 1) * (AidosCurl.HASH_LENGTH / TRYTE_WIDTH / NUMBER_OF_SECURITY_LEVELS); j++) {
    ...
    public static int[] signatureFragment(final int[] normalizedBundleFragment, final int[] keyFragment) {
        ...
        if (normalizedBundleFragment.length != AidosCurl.HASH_LENGTH / TRYTE_WIDTH / NUMBER_OF_SECURITY_LEVELS) {
        ...
        for (int j = 0; j < NUMBER_OF_FRAGMENT_CHUNKS; j++) {
            ...
            for (int k = MAX_TRYTE_VALUE - normalizedBundleFragment[j]; k-- > 0; ) {
                ...
                final AidosCurl hash = new AidosCurl();
                hash.absorb(signatureFragment, j * AidosCurl.HASH_LENGTH, AidosCurl.HASH_LENGTH);
                hash.squeeze(signatureFragment, j * AidosCurl.HASH_LENGTH, AidosCurl.HASH_LENGTH);
            }
        }
    ...
    public static int[] digest(final int[] normalizedBundleFragment, final int[] signatureFragment) {
        ...
        final int[] digest = new int[AidosCurl.HASH_LENGTH];
